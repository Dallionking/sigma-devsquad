...
export const InteractiveTooltip = ({
  ...
  useEffect(() => {
    // Check if user has dismissed this tooltip permanently
  ...
  if (dontShowAgain && localStorage.getItem(`tooltip-dismissed-${id}`) === 'true') {
    return <>{children}</>;
  }
    ...
    <div className={cn("relative inline-block", className)}>
      ...
      {isVisible && (
        <div className="relative">
          <Card className={cn(
            "absolute z-50 w-80 shadow-xl border-0 bg-gray-900 text-white rounded-lg",
            ...
            <CardContent className="p-4">
              <div className="flex items-start justify-between mb-3">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-5 w-5 p-0 text-gray-300 hover:text-white hover:bg-gray-800"
